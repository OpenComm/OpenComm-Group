...
public class CreateAccountController {
	 ...
	 */
	public boolean handleSignUp(String userName, String emailAddress, String name, String passWord){
		...
		boolean invalidFName = !Util.validateString(name, Util.NAME_PATTERN_SAVE);
		boolean emptyEmail = (emailAddress == null || emailAddress.length() == 0);
		boolean invalidEmail = !Util.validateString(emailAddress, Util.EMAIL_ADDRESS_PATTERN);
		...
		if (emptyFName || invalidFName || emptyEmail || invalidEmail || emptyPwd || invalidPwd) {
			StringBuilder errorText = new StringBuilder();
			errorText.append("Error:\n");
			if (emptyFName) errorText.append("\tFirst and last name required\n");
			else if (invalidFName) errorText.append("\tInvalid name\n");
			if (emptyEmail) errorText.append("\tEmail is required\n");
			else if (invalidEmail) errorText.append("\tInvalid email\n");
			if (emptyUser) errorText.append("\tUsername is required\n"); 
		...
		try{
			...
			Toast send = Toast.makeText(this.createAccountView.getApplicationContext(),text,duration);
			send.show(); 
			return true;
		}
		catch(Exception e)
