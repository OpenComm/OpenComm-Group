...
/**
 * This class contains all the actions that one Jingle entity sends to another
...
public class JingleIQActionMessages {
	...
	/**
	 * Allows access to the <code>XMPPConnection</code> object to send IQ
	 * packets
	 * 
	 * @param conn
	...
	/**
	 * Sends out a <code>JingleIQPacket</code> of type
	 * <code>JingleIQPacket.AttributeActionValues.SESSION_INITIATE</code>
	 * 
	 * @param jidFrom
	 *            the "from" attribute of the IQ Packet
	 * @param jidTo
	 *            the "to" attribute of the IQ Packet
	 * @param buddy
	 *            <code>MUCBuddy</code> object to set transport information
	 */
	public void sendSessionInitiate(String jidFrom, String jidTo,
			JingleController buddy) {
		Log.i("JingleIQActionMessages", jidFrom
				+ " sending a session initiate to " + jidTo);
		JingleIQPacket jp = new JingleIQPacket(jidFrom, jidTo,
				JingleIQPacket.AttributeActionValues.SESSION_INITIATE);
		...
		description
				.setAttributeMedia(DescriptionElementType.MediaTypes.TYPE_AUDIO);
		...
		TransportElementType transport = new TransportElementType(
				TransportElementType.TYPE_RAW_UDP);
		CandidateElementType c = new CandidateElementType(
				CandidateElementType.TYPE_LOCAL_CANDIDATE);
		...
		Log.i(JingleIQActionMessages.TAG, "Sending SessionInitiate: "
				+ "From: " + jp.getJIDFrom() + "To: " + jp.getJIDTo()
				+ "Initiator: " + jp.getAttributeInitiator() + "Responder: "
				+ jp.getAttributeResponder());
	...
	/**
	 * Sends out a <code>JingleIQPacket</code> of type
	 * <code>JingleIQPacket.AttributeActionValues.SESSION_TERMINATE</code>
	 * 
	 * @param incomingIQ
	 *            the <code>JingleIQPacket</code> in response to which this
	 *            packet is being sent
	 * @param buddy
	 *            <code>MUCBuddy</code> object to set transport information
	 */
	public void sendSessionAccept(JingleIQPacket incomingIQ,
			JingleController buddy) {

		System.out.println("Now sending out accepting packet!");

		JingleIQPacket jp = new JingleIQPacket(incomingIQ.getTo(),
				incomingIQ.getFrom(),
				JingleIQPacket.AttributeActionValues.SESSION_ACCEPT);
		...
		TransportElementType transport = new TransportElementType(
				TransportElementType.TYPE_RAW_UDP);
		CandidateElementType c = new CandidateElementType(
				CandidateElementType.TYPE_REMOTE_CANDIDATE);
		c.setAttributeComponent((short) 1);
		c.setAttributeGeneration((short) 0);
		// TODO: random generate 10 digit candidate ID
		...
		Log.i(JingleIQActionMessages.TAG,
				"Sending SessionAccept: " + "From: " + jp.getJIDFrom() + "To: "
						+ jp.getJIDTo() + "Initiator: "
						+ jp.getAttributeInitiator() + "Responder: "
						+ jp.getAttributeResponder());
	 ...
	 *            the "to" attribute of the IQ Packet
	 * @param sid
	 *            the <code>SID</code> of the session to be terminated
	 * @param reason
	 *            the reason for termination specified in a
	 *            <code>ReasonElementType</code> object
	 * @param buddy
	 *            <code>MUCBuddy</code> object to end RTP streams with.
	 */

	public void sendSessionTerminate(String from, String to, String sid,
			ReasonElementType reason, JingleController buddy) {
		JingleIQPacket jiqPacket = new JingleIQPacket(from, to,
		...
		if (buddy.receiver.isRunning())
			buddy.receiver.halt();
		if (buddy.sender.isRunning())
			buddy.sender.halt();
		buddy.pusher.removeQueue(buddy.getBuddyJID());
		if (buddy.pusher.isRunning() && buddy.pusher.numQueues() == 0)
			buddy.pusher.halt();
	}
}
