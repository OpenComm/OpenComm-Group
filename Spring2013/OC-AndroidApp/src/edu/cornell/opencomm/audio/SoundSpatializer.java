...
public class SoundSpatializer {
	...
	private void setRegion(int x, int y) {
		// int xCalc = (int) Math.min(10, Math.max(0, Math.floor(((x *
		// 100/Values.screenW) + 5)/10)));
		// int yCalc = (int) Math.min(10, Math.max(0, Math.floor(((y *
		// 100/Values.spaceViewH) + 5)/10)));
		// this.region = xCalc * 11 + yCalc;
		// Log.d(TAG, "coordinate: (" + x + ", " + y + "), coordinate2: (" +
		// xCalc + ", " + yCalc + "), region: " + ((xCalc * 11) + yCalc));
	...
	public short[][] spatializeSource(short[] source, int itd) {
		...
		// this.audio.setStereoVolume(this.vol[0] * SoundSpatializer.spaceVol,
