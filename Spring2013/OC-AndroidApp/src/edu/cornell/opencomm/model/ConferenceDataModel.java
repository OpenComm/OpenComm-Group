...
public class ConferenceDataModel implements InvitationListener {
	...
	/**
	 * The map to hold chat space data model KEY: ChatSpace Room ID VALUE: The
	 * model associated with the chat
	 */
	HashMap<String, ConferenceRoom> conferenceRoomMap;

	/**
	 * The map to hold chat space models in relation to each other. KEY:
	 * ChatSpace Room ID (as assigned when initializing a MUC VALUE: name of
	 * chatspace, e.g., MAIN, LEFT, RIGHT
	 */
	HashMap<Integer, String> tagToRoomIdMap;

	String activeChat;
	/** This is the ID of the current room */
	...
	public ConferenceDataModel(String mainSpaceRoomID, String leftChatID,
			String rightChatID, User u) {
	...
	public ConferenceDataModel(String conferenceTitle, String conferenceTime,
			String conferenceOwner) {
		conferenceRoomMap = new HashMap<String, ConferenceRoom>();
		tagToRoomIdMap = new HashMap<Integer, String>();
		isMain = true;
		// Bind the main room
		createMainRoom(Util.generateUniqueRoomId(conferenceTitle,
				conferenceTime, conferenceOwner));
