	...
	public void multiuserchat() {
		if (chatroomgenerated == false) {
			chat = new MultiUserChat(connection,
					"neeleshroom4@conference.jabber.org");
			try {
				...
				Log.d(TAG,
						"Chatroom not successfully generated, trying to join");
		...
		// If a chat-room has been generated or joined, then creating a
		// PacketListener
		if (chatroomgenerated || chatroomjoined) {
			chat.addMessageListener(new PacketListener() {
// if packet is a message and a message is a groupchat
				public void processPacket(Packet arg0) {
					Log.d(TAG,"packet property as xml," + arg0.toXML());
					
					
					Log.d(TAG, "Can get the message");// Message displayed when
	...
	public void buddylist() {
		...
		for (int i = 0; i < offlineUsersInRoom.size(); i++) {
			...
			Log.d(TAG, "offline users" + offlineUsersInRoom.get(i));
		}
