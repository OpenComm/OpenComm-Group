	...
	public void leaveChat(String sChat) throws XMPPException{
		...
		if (moderator.getUsername().equals(currentUser.getUsername())){    //if moderator
			...
			public class ConferenceController {
				 ...
				 */
				public void transferPrivileges(User u1, User u2,  MultiUserChat room){
					...
					try {
						...
						Log.v("ConferenceController","Error in the nickname of new moderator");
					}
					try {
						room.revokeModerator(u2.getNickname());
					} catch (XMPPException e) {

						Log.v("ConferenceController","Error in the nickname of current moderator");
					}
				}
