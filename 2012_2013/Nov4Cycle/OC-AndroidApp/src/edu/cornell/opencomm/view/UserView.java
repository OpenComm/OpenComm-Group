 ...
 *         NQ[frontend] ISSUES: The border color gets cut off at the top when
 *         drawn. However, on drag the border color is intact. 2. Why does every
 *         user have the same default color? Is hashing working?
 */
public class UserView extends ImageButton implements OCUpdateListener {
	...
	private ConferenceUser person; // The person object this icon is
									// representing
	private Bitmap image = null; // The actual image that will show on the user
									// screen
	...
	public void rescaleSize(int newWidth, int newHeight) {
		...
		image = Bitmap.createBitmap(this.image, 0, 0, width, height, matrix,
				true);
		image = getRoundedCornerBitmap(image, this.borderColor,
				UserView.cornerSize, UserView.borderSize, this.context);
	}
