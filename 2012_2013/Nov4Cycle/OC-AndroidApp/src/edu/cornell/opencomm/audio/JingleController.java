...
public class JingleController {
	...
	private SoundSpatializer soundSpatializer;
		...
		this.buddyJID = user.getUsername().split("@")[0] + "@" + NetworkService.DEFAULT_HOSTNAME +  "/Smack";
	...
	public void processPacket(IQ incomingPacket){
		...
		if (incomingPacket instanceof JingleIQPacket) {
			...
			Log.i("JingleController", "JIQPacket Received in JingleController (" + state.getSessionStateString() + ") : " + 
			...
			Log.d("JingleController", "result acknowledgement sent for JIQPacket with action " + jiqPacket.getAttributeAction());
			...
			if(state.getSessionState() == SessionCallStateMachine.STATE_ENDED){
				if(action.equals(JingleIQPacket.AttributeActionValues.SESSION_INITIATE)){
					...
					if(supportApplication(jiqPacket)){						
						if(supportTransport(jiqPacket)){
							...
							Log.i(JingleController.TAG, "State change to Accept? : " + state.getSessionStateString());
							
							try {
								Log.i("JingleController", "Starting receiver on port " + this.getLocalPort());
								...
								Log.i("JingleController", "Starting sender to " + this.getRemoteIPAddress() + ":" + this.getRemotePort() + " on port " + sendPort);
				...
				if(action.equals(JingleIQPacket.AttributeActionValues.SESSION_ACCEPT)){		
					...
					state.changeSessionState(JingleIQPacket.AttributeActionValues.SESSION_ACCEPT); // sets to Active.
					Log.i(JingleController.TAG, "State after accepting: " + state.getSessionStateString());
			...
			Log.i(JingleController.TAG, "IQ Received in JingleController: From: "  + iq.getFrom()+ 
