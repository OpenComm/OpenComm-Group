...
public class JingleIQBuddyPacketRouter {
	...
	private static final String TAG = "JingleIQBuddyPacketRouter";
		...
		try {
			...
			connection.addPacketListener(new PacketListener() {
				public void processPacket(Packet p) {
					Log.i(JingleIQBuddyPacketRouter.TAG, "Received a packet :: " + p.getFrom() + ", " + p.getTo() + ", " + p.getClass().getSimpleName());
					if (p instanceof JingleIQPacket) {
						...
						if(!JingleController.getUsernameToJingleControllerMap().containsKey(userJid)){
							...
							jCtrl = JingleController.getUsernameToJingleControllerMap().get(userJid);
							jCtrl.setSID(jiq.getAttributeSID());
						...
						if (Space.getMainSpace().getAllParticipants().containsKey(fromJID)) {
							Space.getMainSpace().getAllParticipants().get(fromJID).getjCtrl().processPacket(iq);
						}
