...
public class JingleIQActionMessages {
	private static final String TAG = "JingleIQActionMessages";
	
	private XMPPConnection connection;
	
	private boolean onePublicIP = false;
	 ...
	 */
	public void sendSessionInitiate(String jidFrom, String jidTo, JingleController jCtrl){
		Log.d(TAG, "sending session initiate");
		JingleIQPacket jp = new JingleIQPacket(jidFrom, jidTo, JingleIQPacket.AttributeActionValues.SESSION_INITIATE);
		jCtrl.setSID(jp.getAttributeSID());
		...
		Vector<String> IPs;
		if (onePublicIP) {
			IPs = IPAddresses.getLocalInterfaceIPs();
		}
		else {
			IPs = IPAddresses.getNATedIPs();
		}
		c.setAttributeIP(IPs.get(0));		
		
		Integer port = PortHandler.getInstance().getRecvPort();
		c.setAttributePort(port.shortValue());
		
		jCtrl.setLocalIPAddress(IPs.get(0));
		jCtrl.setLocalPort(port.shortValue());
		...
		Log.i(JingleIQActionMessages.TAG, "***************************************************");
		Log.d(JingleIQActionMessages.TAG,"jp is a jingle packet " + (jp instanceof JingleIQPacket));
	 ...
	 * @param buddy <code>MUCBuddy</code> object to set transport information
	 */
	public void sendSessionAccept(JingleIQPacket incomingIQ, JingleController jCtrl) {
		
		Log.i(JingleIQActionMessages.TAG, "Now sending out accepting packet!");
		...
		//jCtrl.setLocalIPAddress(localIPs.get(0));
		jCtrl.setLocalIPAddress(IPs.get(0));
		jCtrl.setLocalPort(port.shortValue());
		...
		Log.i(JingleIQActionMessages.TAG, "Sending SessionAccept: " + "From: "  + jp.getJIDFrom()+ "To: " + jp.getJIDTo() + 
				 "Initiator: " + jp.getAttributeInitiator() + "Responder: " + jp.getAttributeResponder());
	 ...
	 */
	public void sendSessionTerminate(String from, String to, String sid, ReasonElementType reason, JingleController jCtrl){
		...
		if (jCtrl.receiver.isRunning()) jCtrl.receiver.halt();
		if (jCtrl.sender.isRunning()) jCtrl.sender.halt();
		jCtrl.pusher.removeQueue(jCtrl.getbuddyJID());
		if (jCtrl.pusher.isRunning() && jCtrl.pusher.numQueues() == 0) jCtrl.pusher.halt();
	}
}
