...
public class JingleController {
	...
	public JingleController(User user) {
		...
		this.buddyJID = this.user.getUsername()+ Network.DEFAULT_HOSTNAME + "/Smack";
				...
				if(action.equals(JingleIQPacket.AttributeActionValues.SESSION_INITIATE)){
					...
					if(supportApplication(jiqPacket)){						
						if(supportTransport(jiqPacket)){
							...
							try {
								Log.i(JingleController.TAG, "Starting receiver on port " + this.getLocalPort());
								...
								Log.i(JingleController.TAG, "Starting sender to " + this.getRemoteIPAddress() + ":" + this.getRemotePort() + " on port " + sendPort);
