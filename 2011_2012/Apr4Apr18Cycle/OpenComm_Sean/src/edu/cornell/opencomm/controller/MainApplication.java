...
public final class MainApplication extends Activity {
	...
	public static LinkedList<User> allBuddies; // Your buddy list! Has been
												// previously saved from the
												// network
	 ...
	 * to create a space and therefore make the rest of this code crash. This
	 * usually gives an error 80% of the time, so it is probably linked to the
	...
	public View.OnKeyListener onKeyListener = new View.OnKeyListener() {
		...
		public boolean onKey(View v, int keyCode, KeyEvent event) {
			...
			case KeyEvent.KEYCODE_N: {
				...
				Space.getMainSpace().getKickoutController()
						.kickoutUser(debug, "You suck!");
				break;
			}
			...
			case KeyEvent.KEYCODE_Z: {
				Iterator<String> affiliates = Space.getMainSpace().getMUC()
						.getOccupants();
				while (affiliates.hasNext()) {
					String a = affiliates.next();
					Log.v(TAG, a
							+ "'s affiliaton is "
							+ Space.getMainSpace().getMUC().getOccupant(a)
									.getAffiliation());
				}
				break;
			}
			case KeyEvent.KEYCODE_J: {
				...
				Space.getMainSpace()
						...
						.joined("roomname@" + Network.DEFAULT_CONFERENCE + "/"
								+ debug1.getNickname());
				break;
			}
			case KeyEvent.KEYCODE_S: {
				...
				try {
					SpaceController.addSpace((Context) MainApplication.screen
							.getContext());
				} catch (XMPPException e) {
			...
			case KeyEvent.KEYCODE_B: {
				...
				Space.getMainSpace().getParticipantController()
						.leaveSpace(false);
				break;
			}
			case KeyEvent.KEYCODE_Q: {
				Log.v(TAG,
						"pressed Q key - you (a moderator) received an invite request");
				InvitationController ic = MainApplication.screen.getSpace()
						.getInvitationController();
				// fake invite request
				String inviteRequest = "" + Network.REQUEST_INVITE + "@inviter"
						+ debug.getUsername() + "@invitee"
						+ debug1.getUsername() + "@reason"
			...
			case KeyEvent.KEYCODE_W: {
				...
				InvitationController ic = MainApplication.screen.getSpace()
						.getInvitationController();
				// fake invite request
				String inviteRequest = "" + Network.REQUEST_INVITE
						+ "@requester" + debug.getUsername() + "@invitee"
						+ MainApplication.userPrimary.getUsername() + "@reason"
						+ "Because you're a cool cat.";
				ic.receiveInvitationRequest(inviteRequest);
				/*
			...
			case KeyEvent.KEYCODE_E: {
				Log.v(TAG,
						"pressed E key - you (a moderator) received a kickout request");
				KickoutController kc = MainApplication.screen.getSpace()
						.getKickoutController();
				// fake kickout request
				String kickoutRequest = "" + Network.REQUEST_KICKOUT
						+ "@requester" + debug.getUsername() + "@kickee"
						+ debug1.getUsername() + "@reason"
			...
			case KeyEvent.KEYCODE_R: {
				Log.v(TAG,
						"pressed R key - you received a kickout confirmation");
				KickoutController kc = MainApplication.screen.getSpace()
						.getKickoutController();
				// fake kickout request
				String kickoutRequest = "" + Network.REQUEST_KICKOUT
			...
			case KeyEvent.KEYCODE_H: {
				...
				Space.getMainSpace().getInvitationController()
	...
	public void adjustLayoutParams() {
		...
		// lp = new LinearLayout.LayoutParams(Values.bottomBarH,
		// Values.bottomBarH);
	...
	public void initializeButtons() {
		...
		try {
			...
			Space newSpace = SpaceController.addSpace(Space.getMainSpace()
					.getContext());
			...
			// main chat
			PrivateSpaceIconView main = new PrivateSpaceIconView(Space
					.getMainSpace().getContext(), Space.getMainSpace());
			Space.getMainSpace().getSpaceController().setPSIV(main);
			// sidechat 2
			Space newSpace2 = SpaceController.addSpace(Space.getMainSpace()
					.getContext());
		...
		spatializationButton.setOnClickListener(this
				.getSpatializationListener());
	...
	public void disconnect() {
		...
		for (Space s : Space.getAllSpaces().values()) {
			if (!s.isMainSpace()) {
				if (D)
					Log.v(TAG, "leaving a side chat");
				// s.getParticipantController().leaveSpace(false);
				s.getMUC().leave();
			} else {
				// Leave main space
				if (D)
					Log.v(TAG, "leaving the main space");
				s.getMUC().leave();
			}
		}
		userPrimary = null;
		...
		if (D)
			Log.v(TAG, "starting intent stuff");
	...
	/*
	 * TODO network: 1) Notify network of new private space to create 2) Place
	 * YOU in this privatespace and make you moderator (on network) 3) Should
	 * perhaps return an id# for this space? Put it in the newSpaceID variable,
	 * or you can keep the space_counter that is already implemented
	 */
	/*
	 * int newSpaceID=space_counter++; return createPrivateSpace(true,
	 * isMainSpace, null, String.valueOf(newSpaceID), user_primary); }
	 */
	 ...
	 */
	/*
	 * public Space createPrivateSpace(boolean ICreatedThis, boolean
	 * isMainSpace, LinkedList<User> existing_buddies, String spaceID, User
	 * moderator) throws XMPPException{ // Either your first space (mainspace)
	 * or a newly created space return new Space(this, isMainSpace, spaceID,
	 * moderator); }
	 */
	...
	public void addPrivateSpaceButton(PrivateSpaceIconView psv) {
		...
		lp.setMargins(Values.iconBorderPaddingH, 0, Values.iconBorderPaddingH,
				0);

		psv.setLayoutParams(new LinearLayout.LayoutParams(
				Values.privateSpaceButtonW, Values.privateSpaceButtonW));
		psv.setPadding(Values.iconBorderPaddingH, Values.iconBorderPaddingV,
				Values.iconBorderPaddingH, Values.iconBorderPaddingV);
	...
	public void deletePerson(Space space, User person) throws XMPPException {
		space.getKickoutController().kickoutUser(person,
	...
	public void plusButtonSetUp(int position) {
		...
		ImageView plus = PrivateSpaceIconView.plusSpaceButton(getResources()
				.getColor(R.color.off_white), this);
		...
		plus.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					...
					Space newSpace = SpaceController.addSpace(Space
							.getMainSpace().getContext());
					PrivateSpaceIconView psIcon = new PrivateSpaceIconView(
							Space.getMainSpace().getContext(), newSpace);
					newSpace.getSpaceController().setPSIV(psIcon);
				} catch (XMPPException e) {
					Log.d("MainApplication plusButtonSetUp()",
							"Could not add a Space");
				}
	...
	public View.OnClickListener getSpatializationListener() {
		View.OnClickListener temp = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// To spatialize, find the length each individual has to travel
				// on the quadratic representing the semicircle. Place
				// individual at the x and y location after that travel.

				int numOfPeople = screen.getAllIcons().size();
				Log.v("# of people ", "NUmber of people: " + numOfPeople);
				double lengthOnArcPP = 931.573 / (numOfPeople + 1);
				// http://www.wolframalpha.com/input/?i=y%3D%28455%2F175^2%29%28x-195%29^2%2B25+from+x%3D20+to+x%3D350+length
				// y=(425/175^2)(x-195)^2+25
				// Meanings: 425: the lower bound (425+25 is the lower bound)
				// 175: distance from vertex to lower bound.
				// 195: vertex x-coord
				// 25: vertex y-coord

				int counter = 1;
				for (UserView u : screen.getAllIcons()) {

					// Set x and y loc to beginning
					double x = 20;
					double y = 480.0;
					double needToTravel = lengthOnArcPP * counter;
					double travelled = 0;
					double lastY = 0;
					if (D) {
						Log.v("needToTravel: ", "needToTravel: " + needToTravel);
					}
					while (travelled < needToTravel) {
						// Travelled 0.01 x pixel and however many y pixel
						// corresponding to that, find the length travelled
						// using triangular estimation.
						x = x + 0.01;
						y = (420 * Math.pow((x - 185), 2.0)) / 27225/* 165^2 */+ 25;

						double dy = lastY == 0 ? 0 : y - lastY;
						// Log.v("DY", "dy: " + dy); //Enable this at your
						// peril!
						travelled = travelled
								+ Math.sqrt((Math.pow(0.01, 2.0) + Math.pow(dy,
										2.0)));
						lastY = y;
					}
					if (D) {
						Log.v("Y", "final Y value: " + y + " , final X value: "
								+ x);
					}
					u.setXY((int) x, (int) y);
