...
public class ReceiverThread extends Thread {
	 ...
	 */
	public ReceiverThread(JingleController jCtrl, SipdroidSocket socket) {
		init(socket);
		this.jCtrl = jCtrl;
		this.ss = this.jCtrl.getSoundSpatializer();
		this.track = this.jCtrl.getSoundSpatializer().getAudio();
	}
	...
	public void run() {
		...
		Log.i("ReceiverThread", "started");
		while (running) {
			Log.d("ReceiverThread", "SoundSpatializer volume: " + this.ss.getVolume());
			try {
				rtp_socket.receive(rtp_packet);
				...
				Log.d("ReceiverThread", "timing out: " + rtp_packet.toString());
				if (timeout == 0) {
		...
		if (DEBUG)
			Log.d("ReceiverThread", "rtp receiver terminated");
	}
