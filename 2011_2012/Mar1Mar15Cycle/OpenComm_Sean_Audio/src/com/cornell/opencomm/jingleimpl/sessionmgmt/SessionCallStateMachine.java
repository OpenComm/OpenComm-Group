...
public class SessionCallStateMachine {
	public static String TAG = "SessionCallStateMachine";
	 ...
	 * Constructor
	 */
	public SessionCallStateMachine(String methodName){
		Log.d("SessionCallStateMachine", "method calling new scsm: " + methodName + ": sessioncallstate machine new one is called");
	}
	...
	public boolean changeSessionState(String action){
		...
		case STATE_ENDED:
			if(action.equals(JingleIQPacket.AttributeActionValues.SESSION_INITIATE)){
				...
				Log.i(SessionCallStateMachine.TAG, 
						"STATE_ENDED: changeSessionState succeeded: " + new Boolean(success).toString() + 
						"session State: " + getSessionStateString());
			} else {
				success = false;
				Log.i(SessionCallStateMachine.TAG, 
						"STATE_ENDED: changeSessionState failed: " + new Boolean(success).toString() + 
						"session State: " + getSessionStateString());
			}
			break;
		case STATE_PENDING:
			if(action.equals(JingleIQPacket.AttributeActionValues.SESSION_ACCEPT)){
				...
				success = true;
				Log.i(SessionCallStateMachine.TAG, 
						"STATE_PENDING: changeSessionState succeeded: " + new Boolean(success).toString() + 
						"session State: " + getSessionStateString());
			} else if(action.equals(JingleIQPacket.AttributeActionValues.SESSION_TERMINATE)){
				sessionState = STATE_ENDED;
				success = true;
				Log.i(SessionCallStateMachine.TAG, 
						"STATE_PENDING: changeSessionState failed: " + new Boolean(success).toString() + 
						"session State: " + getSessionStateString());
				...
				success = true;
				Log.i(SessionCallStateMachine.TAG, 
						"EVERYTHING ELSE: changeSessionState succeeded: " + new Boolean(success).toString() + 
						"session State: " + getSessionStateString());
			} else {
				success = false;
				Log.i(SessionCallStateMachine.TAG, 
						"EVERYTHING ELSE: changeSessionState failed: " + new Boolean(success).toString() + 
		...
		case STATE_ACTIVE:
			if(action.equals(JingleIQPacket.AttributeActionValues.SESSION_TERMINATE)){
				...
				success = true;
				Log.i(SessionCallStateMachine.TAG, 
						"Active: changeSessionState succeeded: " + new Boolean(success).toString() + 
						"session State: " + getSessionStateString());
			} else if(action.equals(JingleIQPacket.AttributeActionValues.CONTENT_ACCEPT) ||
					action.equals(JingleIQPacket.AttributeActionValues.CONTENT_ADD) ||
					action.equals(JingleIQPacket.AttributeActionValues.CONTENT_MODIFY) ||
				...
				success = false;
				Log.i(SessionCallStateMachine.TAG, 
						"Active: changeSessionState failed: " + new Boolean(success).toString() + 
						"sessiontState: " + getSessionStateString());
			}
			break;		
		}
