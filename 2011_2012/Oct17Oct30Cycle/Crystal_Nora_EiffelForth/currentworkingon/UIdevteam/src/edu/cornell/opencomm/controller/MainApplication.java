...
public class MainApplication extends Activity{
	...
	public boolean clickedmenu = false;
  ...
  /*ASK: with the built in menu framework there would be no way
   * to customize the menu to look like the design..so I had
   * to detect with key press and then upload a custom view. However,
   * doing it this way doesnt restore the old state after calling
   * onCreate() again, is there a way to do that? Also,
   * could make a menu and give it a layout with the action bar
   * on top as a button with a drawable theme and also make the 
   * elements of the menu drawable attributes too(rectangles)
   */
   public boolean onKeyUp(int keyCode, KeyEvent event)
    ...
    public void addPrivateSpaceButton(PrivateSpaceIconView psv){
		...
		psv.setPadding(viewDimensions.privateSpacePadding, 0, viewDimensions.privateSpacePadding, 0);
