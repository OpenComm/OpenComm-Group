 ...
 */
public class Space {
	// Debugging: set false when not debugging
	...
	private JingleIQActionMessages jiq;
	...
	public Space(Context context, boolean isMainSpace, String roomID, boolean selfCreated/*, User owner*/)
		...
		this.jiq = new JingleIQActionMessages();
		...
		while (occItr.hasNext()) {
			...
			// set up jingle session
			if (!u.getUsername().equals(MainApplication.user_primary.getUsername())) {
				if (D) {
					Log.d("Space", "user MUCBuddy exists now? " + (u.getMUCBuddy() != null));
					Log.d("Space", "username exists now? " + (u.getUsername() != null));
					Log.d("Space", "jiq exists now?" + (jiq != null));
					jiq.sendSessionInitiate(MainApplication.user_primary.getUsername(), u.getUsername(), u.getMUCBuddy());
					Log.d(TAG, "User " + u.getUsername() + " has IP Address: " + u.getMUCBuddy().getRemoteIPAddress());
				}
			}
