...
public class NetworkService {
	...
	public NetworkService(String host, int port) throws XMPPException {
		...
		if (xmppConn.isConnected()) {
			if (D) {
				Log.d(TAG, "XMPP connection established to " + 
						host + " through " + port);
				Log.d(TAG, "Connection ID: " + xmppConn.getConnectionID());
			}
		}
		else {
			if (D) {
		...
		// Let our JingleIQPacketRouter have easy access to the NetworkService instance and XMPPConnection.
        JingleIQBuddyPacketRouter.setNetworkService(this);
        JingleIQBuddyPacketRouter.setup(this.xmppConn);
        Log.d(TAG, "JingleIQBuddyPacketRouter accessed");
