...
public class MUCBuddy {
	private static String TAG = "MUCBuddy";
	...
	public void processPacket(IQ incomingPacket){
		...
		if (incomingPacket instanceof JingleIQPacket) {
			...
			Log.i(MUCBuddy.TAG, "JIQPacket Received in MUCBuddy: " + 
				...
				if(action.equals(JingleIQPacket.AttributeActionValues.SESSION_INITIATE)){
					...
					if(supportApplication(jiqPacket)){						
						if(supportTransport(jiqPacket)){
							...
							Log.i(MUCBuddy.TAG, "State: " + state.getSessionStateString());
					...
					Log.i(MUCBuddy.TAG, "This Combination not supported yet " + 
			...
			Log.i(MUCBuddy.TAG, "IQ Received in MUCBuddy " + "From: "  + iq.getFrom()+ 
