...
public class Space {
    /*
     * Debugging private static boolean D = true;
     */
    ...
    boolean entered = false; // false if you have never opened this space to the
                             // screen
    ...
    public Space(Context context, boolean isMainSpace, String roomID,
            boolean selfCreated/* , User owner */) throws XMPPException {
        ...
        if (selfCreated/* MainApplication.user_primary.equals(owner) */) {
            ...
            this.muc = new MultiUserChat(
                    LoginController.xmppService.getXMPPConnection(),
                    this.roomID);
            this.muc.join(/* owner.getNickname() */MainApplication.user_primary
                    .getNickname());
            ...
            /*
             * //everything after this is experimental List<String> owners = new
             * LinkedList<String>(); owners.add("opencommss@jabber.org");
             * owners.add("opencommsec@jabber.org"); FormField f = new
             * FormField("muc#roomconfig_roomowners");
             * f.setType(FormField.TYPE_JID_MULTI); answerForm.addField(f);
        ...
        // otherwise join as participant
            ...
            this.muc = new MultiUserChat(
                    LoginController.xmppService.getXMPPConnection(), roomID);
        ...
        while (occItr.hasNext()) {
            ...
            if (/* User.getAllUsers().get(occJID) */u != null) {
                ...
                Log.v("Space", "username = " + occJID + ", nickname = "
                        + occJID.substring(0, occJID.indexOf('@')));
                u = new User(occJID, occJID.substring(0, occJID.indexOf('@')),
                        R.drawable.question);
                Log.v("Space", "CHECK: username = " + u.getUsername()
            ...
            Log.d(TAG, "Their affiliation is: "
                    + this.muc.getOccupant(occ).getAffiliation());
            allOccupants.put(occJID, this.muc.getOccupant(occ));
            if (u != MainApplication.user_primary)
                allIcons.add(new UserView(context, u, R.drawable.question,
                        this, start, start));
    ...
    /**
     * Set the entered state of the room: false if you have never opened the
     * space in the spaceview ever
     * 
     * @param entered
     */
