...
public class PrivateSpacePreviewPopup extends LinearLayout {
    ...
    public void setPersonViews(ArrayList<UserView> personViews) {
        ...
        for (UserView personView : personViews) {
            UserView newPersonView = new UserView(personView.getContext(),
                    personView.getImage(), this.getWidth() + k,
                    this.getHeight());
            k += 44;
    ...
    /*
     * Draw everything on the screen including: 1) Background color 2) All icons
     * (need loop, call the icon's draw function in class UserView) 3) Preview
     * (if not null), need to get icons from the SpaceView and draw them
     */
    @Override
