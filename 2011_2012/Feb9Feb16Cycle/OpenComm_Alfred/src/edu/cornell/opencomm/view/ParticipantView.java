...
public class ParticipantView {
    ...
    /*
     * public static void grantOwnership(final Space space, final boolean
     * isMainSpace) { if (context == null) { Log.v(TAG, "Context is null!"); }
     * AlertDialog.Builder builder = new AlertDialog.Builder(context);
     * 
     * class DialogButtonClickHandler implements DialogInterface.OnClickListener
     * { public void onClick(DialogInterface dialog, int clicked) {
     * makeOwner(clicked);
     * MainApplication.screen.getActivity().delPrivateSpaceUI(space,
     * isMainSpace); spaceView.getSpace().getParticipantController().leaveSpace(
     ...
     * 
     * updateParticipants(); builder.setTitle("Please select new owner")
     * .setItems(usersInSpace, new DialogButtonClickHandler())
     * .setNegativeButton("Cancel", new DialogCancelButtonClickHandler())
     * .create(); AlertDialog alert = builder.create(); alert.show(); }
     */

    /*
     * public static void leaveOrDestroy(final Space space) { if (context ==
     * null) { Log.v(TAG, "Context is null!"); } AlertDialog.Builder builder =
     * new AlertDialog.Builder(context);
     * 
     * class DialogButtonClickHandler implements DialogInterface.OnClickListener
     * { public void onClick(DialogInterface dialog, int clicked) { Log.v(TAG,
     * "Clicked = " + clicked); boolean isMainSpace = false;
     * if(space==Space.getMainSpace()) isMainSpace= true; switch (clicked) {
     * case (0): Log.v(TAG, "clicked on Destroy!"); //try {
     * MainApplication.screen.getSpace().getSpaceController().deleteSpace();
     * MainApplication.screen.getActivity().delPrivateSpaceUI(space,
     * isMainSpace); //} catch (XMPPException e) { // Log.v(TAG,
     * "Can't destroy the space!"); //} //if
     * (spaceView.getSpace().equals(Space.getMainSpace())){ Intent i = new
     * Intent(MainApplication.screen.getSpace().getContext(),
     * DashboardView.class);
     * MainApplication.screen.getSpace().getContext().startActivity(i); //}
     * //else{ //TODO: update View to conference //}
     * 
     * break; case (1): grantOwnership(space, isMainSpace); break; } } }
     * updateParticipants();
    ...
    private static void updateParticipants() {
        ...
        while (participantItr.hasNext()) {
            ...
            if (!next.equals(MainApplication.user_primary.getNickname())) {
                Log.v("annie",
                        " "
                                + !next.equals(MainApplication.user_primary
                                        .getNickname()));
                usersInSpace[i++] = next;
            }
        }
    ...
    private static void deleteFromParticipants() {
        for (int i = 0; i < userSelected.length; i++) {
            if (userSelected[i]) {
                User kickMe = MainApplication.screen.getSpace()
                        .getAllNicksnames().get(usersInSpace[i]);
                try {
    ...
    /*
     * private static void makeOwner(int clicked) { String newOwner =
     * MainApplication.screen.getSpace().getAllNicksnames()
     * .get(usersInSpace[clicked]).getUsername();
     * MainApplication.screen.getSpace().getParticipantController()
     * .grantOwnership(newOwner, true); } // end makeOwner method
     */
}
