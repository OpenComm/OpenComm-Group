...
/**
 * A ParticipantStatusListener that listens for the change in status of
 * participants (does not include the primary user -- for primary user, use
 * UserStatusListener) within a space.
 * <p>
 * TODO - alter the internal methods with any necessary UI changes to reflect
 * the network changes that call the methods
 * </p>
 * Information on Roles and Affiliates:
 * <ul>
 * <li>
 * <a href=
 * "http://www.igniterealtime.org/builds/smack/docs/latest/documentation/extensions/muc.html#role"
 * >Roles</a></li>
 * <li>
 * <a href=
 * "http://www.igniterealtime.org/builds/smack/docs/latest/documentation/extensions/muc.html#afiliation"
 * >Affiliates</a></li>
 * </ul>
 */
public class ParticipantStatusController implements ParticipantStatusListener {
     ...
     */
    public void banned(String bannedUserRoomInfo, String banningUser,
            String reason) {
        // DEBUG
        if (D) {
            String[] bannedUserSplit = this
                    .splitUserRoomInfo(bannedUserRoomInfo);
            // check that the split is successful
     ...
     */
    public void kicked(String kickedUserRoomInfo, String kickingUser,
            String reason) {
        Log.v(TAG, "kicked()");
        ...
        // DEBUG
        if (D) {
            // String[] kickedUserSplit =
            // this.splitUserRoomInfo(kickedUserRoomInfo);
            // check that the split is successful
            if (kickedUserSplit != null) {
                // extract the room's name
                ...
                if (D)
                    Log.d(TAG, "addUserStatusListener - "
                            + kickedNickname
                            + " kicked by "
                            + kickingUser
                            + " from room "
                            + kickedRoomname
                            + "\ndue to: "
                            + ((reason == null) ? Network.DEFAULT_KICKOUT
                                    : reason));
            }
        }
    ...
    public void moderatorRevoked(String userRoomInfo) {
        ...
        if (D) {
            ...
            if (userSplit != null) {
                ...
                if (D)
                    Log.d(TAG, "addUserStatusListener - moderator "
                            + "privileges revoked from " + nickname
                            + " in room " + roomname);
            }
        }
     ...
     * 
     * @param info
     *            - string containing room name and nickname
     * @return null if the split does not produce a String array with lengths 2<br>
     *         else a String array in which [0] contains room name, [1] contains
     *         nickname
     */
    public String[] splitUserRoomInfo(String info) {
        ...
        if (userRoomInfoSplit == null || userRoomInfoSplit.length != 2) {
            Log.e(TAG,
                    "addUserStatusListener/splitUserRoomInfo - "
