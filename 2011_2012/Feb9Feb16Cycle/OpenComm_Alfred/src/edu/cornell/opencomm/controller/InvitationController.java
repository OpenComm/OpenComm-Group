...
public class InvitationController implements InvitationRejectionListener {
    ...
    /**
     * Handle when the accept button is pressed 1) If a moderator request - call
     * confirm 2) If an invite to you - then accept
     */
    public void handleAcceptButtonHover() {
        ...
        if (isModeratorRequest) {
            Log.v("InvitationController",
                    "Moderator accepted the invite request");
            confirmInvitationRequest(invite.getInviteInfo());
        } else {
            Log.v("InvitationController", "You accepted the invite request");
            // if you are the only person in the mainspace, then replace your
            // mainspace
            if (Space.getMainSpace().getAllParticipants().size() <= 1)
                SpaceController.swapMainSpace(MainApplication.screen
                        .getContext(), invite.getMUC().getRoom());
            // else create a new space
            else
                SpaceController.addExistingSpace(
                        MainApplication.screen.getContext(), false,
                        invite.getRoom());
        }
    }
     ...
     * 
     * @param mSpace
     *            - The space associated with this controller
     */
     ...
     * =========================================
     * 
     * CUSTOM LISTENERS: receiveInvitationRequest confirmInvitationRequest
     * rejectInvitationRequest receiveInvitationRequestRejection
     * ===========================================
     * ===========================================
     */

    /**
     * Sends an invitation user to a room. If the primary user is the owner of
     * the room, it sends the invitee an invitation to the space. If the primary
     * user is not, the user sends a request to the room requesting an
     * invitation request
     * 
     * @param invitee
     *            - user to invite
     * @param reason
     *            - reason for inviting the user to the space
     *            <p>
     *            TODO when receiving messages, check all messages for the
     *            InviteRequest tag (Network.REQUEST_INVITE); when it's shown,
     *            do not show the message; rather, call the method
     *            confirmInvitationRequest method
     *            </p>
     */
    public void inviteUser(User invitee, String reason) {
        ...
        if (mSpace.getOwner().equals(MainApplication.user_primary)) {
            ...
            this.mSpace.getMUC().invite(invitee.getUsername(),
        ...
        // send message to owner invitation request
            ...
            // message containing invite request tag, the username of the
            // inviter,
     ...
     * invite a user to a space in String array format {inviterJID, inviteeJID,
     * inviteReason}
     * 
     * @param inviteRequest
    ...
    public String[] receiveInvitationRequest(String inviteRequest) {
        ...
        if (inviteRequest.contains(Network.REQUEST_INVITE)) {
            ...
            invitationView = new InvitationView(inflater, new Invitation(
                    inviteInfo, true), this);
    ...
    public void rejectInvitationRequest(String[] inviteInfo, String reason) {
        ...
        if (inviteInfo != null && inviteInfo.length == 3) {
            ...
            Message msg = this.mSpace.getMUC().createMessage();
            msg.setBody(Network.REJECT_INVITE + "@inviter" + inviteInfo[0]
                    + "@invitee" + inviteInfo[1] + "@reason" + inviteInfo[2]
                    + "@rejectreason"
     ...
     * 
     * @param inviteRequest
     *            - message sent to the room when a owner rejects an invitation
     *            request user to a room. Contains the Reject_Invite Tag<br>
     *            (format: (Network.REJECTT_INVITE)@inviter(Inviter's JID)@invitee
     *            (Invitee's JID)@reason(Invitation
     *            reason)@rejectionreason(Rejection reason)
     * 
     * @return Information of rejection of invitation request received from the
     *         owner of the room in String array format {inviterJID, inviteeJID,
     *         inviteReason, rejectionReason}
     */
    public String[] receiveInvitationRequestRejection(String requestReject) {
        ...
        if (requestReject.contains(Network.REJECT_INVITE)) {
            ...
            if (MainApplication.user_primary.getUsername().contains(inviter)) {
                String invitee = (requestRejectInfo.split("@inviter" + inviter
                ...
                String[] rejectInfo = { inviter, invitee, reason, rejectReason };
     ...
     * 
     * @param inviter
     *            - Person who invited you to the chat
     */
    public void decline(Invitation invitation) {
        MultiUserChat.decline(invitation.getConnection(), invitation.getRoom(),
    ...
    MultiUserChat accept(String username) throws XMPPException {
        MultiUserChat chat = new MultiUserChat(this.invitation.getConnection(),
                this.invitation.getRoom());
        chat.join(username);
        return chat;
    }
    ...
    /**
     * Automagically called when an invitation this client sent was rejected (oh
     * snap!)
     */
    @Override
