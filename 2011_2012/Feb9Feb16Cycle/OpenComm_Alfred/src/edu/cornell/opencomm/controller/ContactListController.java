        ...
        for (int i = 0; i < buddySelection.length; i++) {
            if (buddySelection[i]) {
                ...
                User p = new User(username + "@jabber.org", username,
                        R.drawable.question);
                Log.v("ContactListController", "addFromBuddyList this space = "
                        + MainApplication.screen.getSpace());
                MainApplication.screen.getSpace().getInvitationController()
                        .inviteUser(p, Network.DEFAULT_INVITE);
    ...
    public static void updateBuddyList() {
        ...
        if (inMainSpace) {
            ...
            buddyList = new CharSequence[entryCollection.size()
                    - Space.getMainSpace().getAllIcons().size() + 1];
            // Log.v("ContactListController", "entry size " +
            // entryCollection.size());
            // Log.v("ContactListController", "space size " +
            // Space.getMainSpace().getAllIcons().size());
            int i = 0;
            while (entryItr.hasNext()) {
                String nickname = entryItr.next().getUser().split("@")[0];
                if (!Space.getMainSpace().getAllNicksnames()
        ...
        // mainchat
            ...
            Log.v("ContactListController", "mainspace = "
                    + Space.getMainSpace().getAllIcons().size()
                    + ", sidechat = "
                    + MainApplication.screen.getSpace().getAllIcons().size());
            ...
            while (participantItr.hasNext()) {
                ...
                if (!(next == MainApplication.user_primary.getNickname() || MainApplication.screen
                        .getSpace().getAllNicksnames().containsKey(next))) {
                    buddyList[i++] = next;
                }
            }
        }
