...
public class MessageController {
     ...
     * 
     * @param mSpace
     *            - the space within which the messages are sent and received
     */
    ...
    public PacketListener configMessageListener() {
        PacketListener messageListener = new PacketListener() {
            ...
            public void processPacket(Packet packet) {
                if (D)
                    Log.d(TAG, "configMessageListener - processPacket");
                ...
                if (mSpace.getOwner().equals(MainApplication.user_primary)) {
                    ...
                    if (body.contains(Network.REQUEST_INVITE)) {
                        if (D)
                            Log.d(TAG,
                                    "configMessageListener - invitation request received");
                        mSpace.getInvitationController()
                                .receiveInvitationRequest(body);
                    }
                    // if it is a kickout request:
                    else if (body.contains(Network.REQUEST_KICKOUT)) {
                        if (D)
                            Log.d(TAG,
                                    "configMessageListener - kickout request received");
                        mSpace.getKickoutController().receiveKickoutRequest(
                                body);
                    }
                    ...
                    // normal messages
                        ...
                        if (D)
                            Log.d(TAG, "Regular message receive from " + from
                                    + ": " + body);
                    }
                ...
                // if the primary user is not the owner of the room
                    ...
                    if (body.contains(Network.REJECT_INVITE)) {
                        String[] receive = mSpace.getInvitationController()
                                .receiveInvitationRequestRejection(body);
                        // TODO: Trigger update to view
                    }
                    // listen for rejection of invitation request
                    else if (body.contains(Network.REJECT_KICKOUT)) {
                        String[] receive = mSpace.getKickoutController()
                                .receiveKickoutRequestRejection(body);
                        // TODO: Trigger update to view
                    }
                    // listen for delete requests
                        ...
                        if (D)
                            Log.d(TAG,
                                    "configMessageListener - delete request received");
                        String requester = (body.split("@requester")[0])
                                .split("@deletee")[0];
