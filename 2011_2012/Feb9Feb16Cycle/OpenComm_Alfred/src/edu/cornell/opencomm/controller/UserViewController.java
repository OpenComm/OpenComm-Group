...
public class UserViewController {
    ...
    /**
     * When a user presses down on a UserView, save the initial icon position
     * 
     * @param clickX
    ...
    /**
     * After a click up on an icon. Handle these 2 cases: 1) If the icon is
     * clicked up over the bottom bar, then return the icon to its original
     * position before moving it 2) If icon is clicked up over the spaceview,
     * then change the icon's position
     * 
     * @param clickX
     *            - placed position of icon center before dragging
     * @param clickY
     *            - placed position of icon center before dragging
     */
    public void handleClickUp(int clickX, int clickY) {
        int newX, newY;
        Log.v("UserViewController", "y is " + clickY + ", area is "
                + (Values.spaceViewH - Values.userIconH / 2));
        // 1)
